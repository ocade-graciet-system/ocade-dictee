...
> **ℹ️ OCADE-HANDY est un fork de [Handy](https://github.com/cjpais/Handy) (cjpais/Handy), sous licence MIT.**
> Ce dépôt ajoute notamment une chaîne de release automatique (installeur `.exe` Windows et `.dmg` macOS générés sur GitHub Actions). Voir [Releases automatiques](#releases-automatiques).
...
| Ta machine                                                                            | Fichier            |
| ------------------------------------------------------------------------------------- | ------------------ |
| **Windows** (64 bits — la quasi-totalité des PC, y compris Windows ARM via émulation) | `…_x64-setup.exe`  |
| **Mac Apple Silicon** (M1, M2, M3, M4…)                                               | `…_aarch64.dmg`    |
| **Mac Intel** (modèles ≈ avant 2020)                                                  | `…_x64.dmg`        |
| **Linux** (toutes distributions, x64)                                                 | `…_amd64.AppImage` |

Les binaires ne sont **pas signés** (pas de certificat payant) — au premier lancement :

- **Windows** : écran « Windows a protégé votre ordinateur » → clique **« Informations complémentaires »** puis **« Exécuter quand même »**. Si l'antivirus met le fichier en quarantaine, restaure-le et ajoute une exclusion.
- **macOS** : glisse l'app dans **Applications**, puis au premier lancement, si « l'app n'a pas pu être vérifiée » : ouvre **Réglages Système → Confidentialité et sécurité**, descends jusqu'au message concernant OCADE Dictée et clique **« Ouvrir quand même »** (sur les anciens macOS : clic droit → Ouvrir → Ouvrir suffit). En dernier recours, dans le Terminal : `xattr -dr com.apple.quarantine "/Applications/OCADE Dictée.app"`. Pense aussi à **éjecter le volume du DMG** après l'installation et à **supprimer une éventuelle ancienne app « Handy »** dans Applications.
- **Linux** : rends l'AppImage exécutable (`chmod +x` ou clic droit → Propriétés → « Autoriser l'exécution ») puis double-clique. Support Wayland limité — préférer une session X11/Xorg.

Ensuite : accorde les permissions système (micro, accessibilité), configure ton raccourci dans les réglages, et dicte !

**Mises à jour** : cette installation manuelle n'est nécessaire **qu'une seule fois**. L'app vérifie ensuite les nouvelles versions au démarrage et propose un bouton de mise à jour intégré (Windows, macOS, Linux) — les mises à jour installées par l'app ne redéclenchent pas les avertissements de sécurité ci-dessus.
...
- **Windows** : installeur `.exe` (NSIS, x64 uniquement — tourne aussi sur Windows ARM via émulation)
- **macOS** : image disque `.dmg` (Apple Silicon et Intel)
- **Linux** : `.AppImage` (universel, x64)

4 fichiers seulement, un par machine cible, avec un guide d'installation en français généré dans le corps de la release.
