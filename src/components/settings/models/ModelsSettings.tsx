...
import { isLegacySource, ModelCard } from "@/components/onboarding";
...
// "ggml" as a searchable token for the Whisper `.bin` weights that are in that
// format: it is how users refer to those files, yet the word appears in no
// display name and in few filenames (`whisper-distil-fr-dec2-q5_0.bin`). GGUF
// needs no counterpart — searching "gguf" already matches via the filename,
// which is in the same haystack.
const ggmlToken = (filename: string): string =>
  filename.endsWith(".bin") ? "ggml" : "";

// Reference catalog of ready-to-use Whisper GGML/GGUF files to try.
const HUGGING_FACE_MODELS_URL = "https://huggingface.co/ggerganov/whisper.cpp";
...
export const ModelsSettings: React.FC = () => {
  ...
  // Map the backend's stable import error codes to localized messages.
  const importErrorMessage = (error: string): string => {
    if (error.includes("invalid_extension")) {
      return t("settings.models.import.errors.invalidExtension");
    }
    if (error.includes("file_exists")) {
      return t("settings.models.import.errors.fileExists");
    }
    if (error.includes("reserved_filename")) {
      return t("settings.models.import.errors.reservedFilename");
    }
    return t("settings.models.import.errors.generic");
  };
  ...
  const handleImportModel = async () => {
    ...
    const result = await importModel(selected);
    if (result.ok) {
      toast.success(t("settings.models.import.success"));
    } else {
      toast.error(importErrorMessage(result.error));
    }
  };

  // Filter models by search query and language filter. The haystack spans name,
  // description, filename and id, because a model is as often known by its file
  // as by its display name ("ggml", "q5_0", "dec2") — and neither filename nor
  // id is surfaced anywhere else in this screen.
  const filteredModels = useMemo(() => {
    ...
    return models.filter((model: ModelInfo) => {
      ...
      if (q) {
        const haystack =
          `${model.name} ${model.description} ${model.filename} ${model.id} ${ggmlToken(model.filename)}`.toLowerCase();
  ...
  return (
    <div className="max-w-3xl w-full mx-auto space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {/* Import a model file without touching the models folder */}
                <button
                  type="button"
                  onClick={handleImportModel}
                  disabled={isImporting}
                  title={t("settings.models.import.tooltip")}
                ...
                >
                  ...
                  />
                  <span>{t("settings.models.import.label")}</span>
                </button>
      ...
      {/* Where to find more model files to import */}
      <button
        type="button"
        onClick={() => openUrl(HUGGING_FACE_MODELS_URL)}
        className="flex items-center gap-1.5 text-sm text-text/60 hover:text-logo-primary transition-colors"
      >
        <ExternalLink className="w-3.5 h-3.5" />
        <span>{t("settings.models.findMoreModels")}</span>
      </button>
    </div>
  );
};
