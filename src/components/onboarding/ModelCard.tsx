...
// Legacy = an *upstream blob* (blob.handy.computer) .bin/ONNX download,
// superseded by the catalog GGUFs: kept runnable when already on disk, but no
// longer advertised. Url-sourced models hosted elsewhere — the fork's Hugging
// Face additions, e.g. the default French distil — are NOT legacy: they carry no
// "legacy" badge and stay offered in onboarding.
//
// Single source of truth for the whole app; the models settings screen imports
// this rather than re-deriving it (the two drifted apart once already).
export const isLegacySource = (model: ModelInfo): boolean =>
  typeof model.source === "object" &&
  "Url" in model.source &&
  model.source.Url.url.includes("blob.handy.computer");
