        ...
        in
        {
          handy = pkgs.rustPlatform.buildRustPackage {
            ...
            postPatch = ''
              ...
              # Sidecar yt-dlp (bundle.externalBin) : le sandbox est hermétique
              # (pas de réseau ni curl), on fournit le binaire depuis nixpkgs —
              # scripts/fetch-yt-dlp.ts (beforeBuildCommand) se no-op dès que le
              # binaire du target est présent.
              mkdir -p src-tauri/binaries
              cp ${pkgs.yt-dlp}/bin/yt-dlp \
                src-tauri/binaries/yt-dlp-${pkgs.stdenv.hostPlatform.rust.rustcTarget}
