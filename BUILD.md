...
> **Note (sidecar yt-dlp) :** la transcription depuis une URL embarque le
> binaire `yt-dlp` (déclaré dans `bundle.externalBin`). Les hooks
> `beforeDevCommand`/`beforeBuildCommand` le téléchargent automatiquement dans
> `src-tauri/binaries/` via `bun scripts/fetch-yt-dlp.ts`. Si vous lancez
> `cargo build`/`cargo test` directement (hors `bun tauri ...`), exécutez ce
> script une fois d'abord — `tauri-build` exige la présence du sidecar dès la
> compilation.
