...
/// OCADE — filet de sécurité : force le texte transcrit dans le presse-papiers.
/// Utilisé quand le collage automatique échoue, pour ne JAMAIS perdre le texte
/// dicté (l'utilisateur peut alors coller manuellement avec Cmd+V).
pub fn save_to_clipboard(text: &str, app_handle: &AppHandle) -> Result<(), String> {
    app_handle
        .clipboard()
        .write_text(text.to_string())
        .map_err(|e| format!("Failed to write transcription to clipboard: {}", e))
}
