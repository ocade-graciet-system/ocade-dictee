...
pub fn initialize_enigo(app: AppHandle) -> Result<(), String> {
    ...
    // Try to initialize. `Enigo::new()` lit la disposition clavier via les API
    // TIS/TSM, qui exigent le main thread sur macOS 15+ (sinon dispatch_assert_queue
    // tue le process). Cette commande tourne sur un thread worker Tauri, donc on
    // délègue explicitement la construction au main thread.
    match EnigoState::new_on_main_thread(&app) {
        Ok(enigo_state) => {
