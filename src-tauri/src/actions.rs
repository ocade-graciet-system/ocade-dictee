...
impl ShortcutAction for TranscribeAction {
    fn start(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        let model_supports_streaming = selected_model_info
            ...
            .unwrap_or(false);
        // OCADE — VAD neutralisé en dur. Le VAD (Silero) écartait parfois tout
        // l'audio jugé « non-parlé », d'où une transcription vide et « rien
        // écrit » malgré la barre d'écoute. On capture désormais toujours
        // l'intégralité de la voix (le toggle a été retiré de l'UI).
        let _ = settings.vad_enabled;
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some(samples) = rm.stop_recording(&binding_id, cancel_generation) {
                ...
                if samples.is_empty() {
                    ...
                    // OCADE — ne jamais échouer en silence : prévenir l'utilisateur.
                    let _ = ah.emit("transcription-empty", ());
                    ...
                    match transcription_result {
                        Ok(transcription) => {
                            ...
                            if processed.final_text.is_empty() {
                                // OCADE — jamais d'échec silencieux : prévenir l'utilisateur.
                                let _ = ah.emit("transcription-empty", ());
                                utils::hide_recording_overlay(&ah);
                                change_tray_icon(&ah, TrayIconState::Idle);
                            } else {
                                ...
                                // OCADE — filet anti-perte : copie conservée pour le cas où
                                // l'exécution sur le main thread échoue (la closure ne tourne
                                // alors jamais).
                                ...
                                ah.run_on_main_thread(move || {
                                    ...
                                    // Copie pour le filet si le collage échoue.
                                    let paste_fallback = final_text.clone();
                                    match utils::paste(final_text, ah_clone.clone()) {
                                        ...
                                        Err(e) => {
                                            ...
                                            // Ne jamais perdre le texte : le laisser au presse-papiers.
                                            let _ = crate::clipboard::save_to_clipboard(
                                                &paste_fallback,
                                                &ah_clone,
                                            );
                                ...
                                .unwrap_or_else(|e| {
                                    ...
                                    // La closure n'a pas tourné → sauver le texte au
                                    // presse-papiers pour ne pas le perdre + prévenir.
                                    let _ =
                                        crate::clipboard::save_to_clipboard(&fallback_text, &ah);
