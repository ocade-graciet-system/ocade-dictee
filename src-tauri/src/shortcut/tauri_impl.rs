...
/// Vrai si `raw` peut réellement être enregistré comme raccourci global Tauri :
/// validation métier ([`validate_shortcut`]) **et** parsing effectif par le plugin
/// (`parse::<Shortcut>`). Sert de vérité-terrain à la migration des bindings, pour
/// distinguer un raccourci simplement latéralisé (récupérable par
/// dé-latéralisation) d'un raccourci intrinsèquement incompatible (modificateur
/// seul, touche inconnue).
pub fn can_register(raw: &str) -> bool {
    validate_shortcut(raw).is_ok() && raw.parse::<Shortcut>().is_ok()
}
