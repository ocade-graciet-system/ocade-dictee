...
pub fn init_shortcuts(app: &AppHandle) {
    // Auto-répare d'abord les bindings incompatibles avec l'implémentation active
    // (ex. settings édité à la main : bascule vers `tauri` avec des raccourcis
    // latéralisés). Migre ou retombe sur le défaut, puis persiste — pour que
    // l'enregistrement ci-dessous parte toujours de bindings valides.
    heal_bindings_for_active_implementation(app);
...
/// Retire les suffixes de latéralité (`_left`/`_right`) de chaque token d'un
/// raccourci. `handy_keys` produit des modificateurs latéralisés (ex.
/// `option_left`) que le backend `tauri` ne sait pas parser ; les dé-latéraliser
/// préserve l'intention de l'utilisateur (mêmes modificateurs + même touche
/// principale) au lieu de tout réinitialiser.
fn delateralize_shortcut(raw: &str) -> String {
    raw.split('+')
        .map(|tok| {
...
/// Tente de migrer un binding vers `target` **sans le réinitialiser** au défaut.
///
/// - `Some(v)` avec `v == raw` : déjà valide, rien à changer.
/// - `Some(v)` avec `v != raw` : migré (ex. dé-latéralisé), à persister.
/// - `None` : intrinsèquement incompatible → l'appelant retombe sur le défaut.
///
/// Couvre le bug de bascule `handy_keys` → `tauri` : un binding latéralisé comme
/// `option_left+command_left+r` devient `option+command+r` au lieu de faire
/// échouer l'enregistrement.
pub(crate) fn migrate_binding_for_implementation(
    raw: &str,
    target: KeyboardImplementation,
) -> Option<String> {
    match target {
        // handy_keys accepte modificateur-seul, latéralisé et fn : rien à migrer.
        KeyboardImplementation::HandyKeys => handy_keys::validate_shortcut(raw)
            .ok()
            .map(|_| raw.to_string()),
        KeyboardImplementation::Tauri => {
            if tauri_impl::can_register(raw) {
                return Some(raw.to_string());
            }
            // Dé-latéraliser puis revérifier via le parsing réel du plugin.
            let candidate = delateralize_shortcut(raw);
            if candidate != raw && tauri_impl::can_register(&candidate) {
                Some(candidate)
            } else {
                None
            }
...
/// Résultat de la résolution d'un binding pour une implémentation donnée.
enum ResolvedBinding {
    /// Déjà valide : ne rien changer.
    Unchanged,
    /// Migré (ex. dé-latéralisé) vers une valeur équivalente valide.
    Migrated(String),
    /// Intrinsèquement incompatible : réinitialisé au binding par défaut fourni.
    Reset(String),
}

/// Décide, avec le logging associé, ce qu'il advient de `current` pour `target` :
/// inchangé, migré, ou réinitialisé à `default`. Décision **partagée** par
/// l'auto-réparation au démarrage et la bascule d'implémentation, pour qu'elles
/// n'aient qu'un seul comportement à faire évoluer.
fn resolve_binding_for_implementation(
    id: &str,
    current: &str,
    default: &str,
    target: KeyboardImplementation,
) -> ResolvedBinding {
    match migrate_binding_for_implementation(current, target) {
        Some(v) if v == current => ResolvedBinding::Unchanged,
        Some(v) => {
            info!("Binding '{id}' migré pour {target:?}: '{current}' → '{v}'");
            ResolvedBinding::Migrated(v)
        }
        None => {
            warn!(
                "Binding '{id}' incompatible avec {target:?} ('{current}'), \
                 réinitialisé au défaut '{default}'"
            );
            ResolvedBinding::Reset(default.to_string())
        }
    }
}

/// Auto-répare, au démarrage, les bindings incompatibles avec l'implémentation
/// active, puis persiste les corrections. Indispensable lorsque `settings_store.json`
/// a été édité à la main (ex. `keyboard_implementation` basculé sur `tauri` alors
/// que les raccourcis sont encore latéralisés) : sans ça, l'enregistrement échoue
/// silencieusement et le raccourci ne marche plus. On migre quand c'est possible,
/// sinon on retombe proprement sur le défaut.
fn heal_bindings_for_active_implementation(app: &AppHandle) {
    let mut settings = settings::get_settings(app);
    let implementation = settings.keyboard_implementation;

    // `handy_keys` accepte tout (latéralisé, modificateur-seul, fn) : la migration
    // y est toujours un no-op. On évite donc toute la passe (désérialisation des
    // défauts + boucle) pour l'implémentation par défaut, sur le chemin de démarrage.
    ...
    let ids: Vec<String> = settings.bindings.keys().cloned().collect();
    for id in ids {
        // Le raccourci d'annulation est enregistré dynamiquement, hors périmètre.
        if id == "cancel" {
            continue;
        }
        ...
        let new_value = match resolve_binding_for_implementation(
            &id,
            &binding.current_binding,
            &default,
            implementation,
        ) {
            ResolvedBinding::Unchanged => continue,
            ResolvedBinding::Migrated(v) | ResolvedBinding::Reset(v) => v,
        };

        let mut updated = binding;
        updated.current_binding = new_value;
...
fn register_all_shortcuts_for_implementation(
    ...
    // Persistance nécessaire aussi bien pour une migration que pour un reset.
    ...
    for (id, default_binding) in &default_bindings {
        ...
        // Tenter d'abord une migration (dé-latéralisation) avant de réinitialiser :
        // un binding latéralisé récupérable est migré silencieusement ; seul un
        // binding intrinsèquement incompatible est réinitialisé au défaut et
        // signalé à l'UI via `reset_bindings`. Décision factorisée avec
        // l'auto-réparation au démarrage.
        match resolve_binding_for_implementation(
            id,
            &binding.current_binding,
            &default_binding.current_binding,
            implementation,
        ) {
            ResolvedBinding::Unchanged => {}
            ResolvedBinding::Migrated(v) => {
                binding.current_binding = v;
                current_settings
                    .bindings
                    .insert(id.clone(), binding.clone());
                settings_changed = true;
            }
            ResolvedBinding::Reset(v) => {
                binding.current_binding = v;
                current_settings
                    .bindings
                    .insert(id.clone(), binding.clone());
    ...
    // Persiste si des bindings ont été migrés et/ou réinitialisés.
    if settings_changed {
...
#[tauri::command]
#[specta::specta]
pub fn change_mute_others_while_recording_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.mute_others_while_recording = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_comm_mute_shortcut_setting(app: AppHandle, shortcut: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.comm_mute_shortcut = shortcut;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_comm_mute_mode_setting(
    app: AppHandle,
    mode: crate::settings::CommMuteMode,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.comm_mute_mode = mode;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[cfg(test)]
mod migration_tests {
    use super::{delateralize_shortcut, migrate_binding_for_implementation};
    use crate::settings::KeyboardImplementation;

    #[test]
    fn delateralize_strips_side_suffixes() {
        assert_eq!(
            delateralize_shortcut("option_left+command_left+r"),
            "option+command+r"
        );
        assert_eq!(
            delateralize_shortcut("ctrl_right+shift_left+k"),
            "ctrl+shift+k"
        );
    }

    #[test]
    fn delateralize_leaves_plain_tokens_untouched() {
        assert_eq!(delateralize_shortcut("option+space"), "option+space");
        assert_eq!(delateralize_shortcut("f13"), "f13");
    }

    #[test]
    fn migrate_delateralizes_lateralized_binding_for_tauri() {
        // Le bug d'origine : option_left+command_left+r doit devenir option+command+r
        // au lieu de faire échouer l'enregistrement.
        assert_eq!(
            migrate_binding_for_implementation(
                "option_left+command_left+r",
                KeyboardImplementation::Tauri
            )
            .as_deref(),
    ...
    #[test]
    fn migrate_keeps_already_valid_tauri_binding() {
        assert_eq!(
            migrate_binding_for_implementation("option+space", KeyboardImplementation::Tauri)
                .as_deref(),
            Some("option+space")
        );
    }
    ...
    #[test]
    fn migrate_rejects_modifier_only_for_tauri() {
        // Tauri ne supporte pas le modificateur-seul → None (retombe sur le défaut).
        assert_eq!(
            migrate_binding_for_implementation("option_left", KeyboardImplementation::Tauri),
            None
        );
    }

    #[test]
    fn migrate_handy_keys_accepts_lateralized_unchanged() {
        assert_eq!(
            migrate_binding_for_implementation(
                "option_left+command_left+r",
                KeyboardImplementation::HandyKeys
            ...
            .as_deref(),
            Some("option_left+command_left+r")
        );
    }
}
