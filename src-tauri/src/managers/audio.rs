...
impl AudioRecordingManager {
    ...
    pub fn new(
        ...
        let manager = Self {
            ...
            did_comm_mute: Arc::new(Mutex::new(false)),
    ...
    /// Envoie le raccourci `comm_mute_shortcut` dans la direction donnée via
    /// enigo. Retourne `true` si l'envoi a réussi (raccourci valide, EnigoState
    /// disponible et verrouillable). L'appelant tient déjà `did_comm_mute` :
    /// l'ordre de verrous reste `did_comm_mute` → EnigoState.
    fn send_comm_mute_shortcut(&self, shortcut: &str, direction: enigo::Direction) -> bool {
        let keys = match crate::input::parse_shortcut_to_keys(shortcut) {
            Ok(k) => k,
            Err(e) => {
                debug!("comm mute: raccourci invalide: {e}");
                return false;
            }
        };
        // `enigo.key(Key::Unicode(..))` traduit le caractère en keycode via les API
        // TIS, qui exigent le main thread sur macOS 15+ (sinon le process est tué).
        // apply_mute/remove_mute tournent dans des threads secondaires, donc bloquer
        // le temps de l'exécution sur le main thread est sûr (pas de deadlock).
        let app = self.app_handle.clone();
        crate::input::run_on_main_thread_blocking(&self.app_handle, move || {
            ...
            crate::input::send_shortcut(&mut enigo, &keys, direction).is_ok()
        })
        .unwrap_or(false)
    }

    /// Envoie le raccourci de mute à l'app de com active si activé.
    /// Push-to-mute: Press (maintenu jusqu'au stop). Toggle: Click.
    ///
    /// Doit être appelé de façon SYNCHRONE au démarrage de l'enregistrement
    /// (contrairement à `apply_mute`, différé sur un thread) : en push-to-mute,
    /// un appui bref ne doit pas pouvoir laisser un `Press` sans `Release` si le
    /// stop/cancel arrive avant que l'application du mute ait eu lieu.
    pub fn apply_comm_mute(&self) {
        let settings = get_settings(&self.app_handle);
        if !settings.mute_others_while_recording {
            return;
        }
        let mut guard = self.did_comm_mute.lock().unwrap();
        let dir = match settings.comm_mute_mode {
            crate::settings::CommMuteMode::PushToMute => enigo::Direction::Press,
            crate::settings::CommMuteMode::Toggle => enigo::Direction::Click,
        };
        if self.send_comm_mute_shortcut(&settings.comm_mute_shortcut, dir) {
            *guard = true;
            debug!("comm mute appliqué");
        }
    }

    /// Réactive le micro dans l'app de com si un mute avait été appliqué.
    pub fn remove_comm_mute(&self) {
        let mut guard = self.did_comm_mute.lock().unwrap();
        if !*guard {
            return;
        }
        let settings = get_settings(&self.app_handle);
        let dir = match settings.comm_mute_mode {
            crate::settings::CommMuteMode::PushToMute => enigo::Direction::Release,
            crate::settings::CommMuteMode::Toggle => enigo::Direction::Click,
        };
        // Le mute est retiré quoi qu'il arrive (guard remis à false) pour ne
        // jamais rester bloqué, même si l'envoi échoue.
        let _ = self.send_comm_mute_shortcut(&settings.comm_mute_shortcut, dir);
        *guard = false;
        debug!("comm mute retiré");
    }
