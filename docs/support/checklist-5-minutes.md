...
- **Téléchargement bloqué ou interrompu** : le message nomme la ressource concernée (moteur, depuis `github.com` ; modèle, depuis `huggingface.co`) et la cause — aucune connexion, nom du serveur non résolu, connexion refusée, certificat rejeté, délai dépassé. Un seul des deux hébergeurs est souvent filtré : si le moteur passe et pas le modèle, la connexion du poste n'est pas en cause, c'est `huggingface.co` qu'il faut autoriser (pare-feu, proxy d'entreprise, antivirus). Dépliez **Détail technique** sous le message et faites-le copier au client (bouton à droite) : c'est la chaîne de causes exacte, également présente dans le journal. Une fois la cause levée, cliquez à nouveau sur **Résumer** : le téléchargement reprend là où il s'était arrêté. Sur Windows, un antivirus peut aussi analyser le fichier de 2,2 Go pendant plusieurs minutes au premier usage.
